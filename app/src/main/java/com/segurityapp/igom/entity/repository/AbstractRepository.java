...
public abstract class AbstractRepository<T> implements RepositoryInterface<T> {
    ...
    protected void verifyAccessToken() {
        ...
        if (c.moveToLast()) {
            id = c.getString(c.getColumnIndex(AccessTokenContract.AccessTokenEntry.COLUMN_NAME_ID));
            accessTokenStr = c.getString(c.getColumnIndex(AccessTokenContract.AccessTokenEntry.COLUMN_NAME_ACCESS_TOKEN));
            expiresAt = c.getString(c.getColumnIndex(AccessTokenContract.AccessTokenEntry.COLUMN_NAME_ACCESS_TOKEN_EXPIRE_AT));
        }
    ...
    private void callApi() {
        ...
        adapter.getHttp().newCall(adapter.getAdapter()).enqueue(new Callback() {
            ...
            public void onResponse(@NonNull Call call, @NonNull Response response) throws IOException {
                ...
                AccessToken accessToken = (AccessToken) jsonAdapter.parseToEntityObject();
